...
export default function GameCard({ game, currentUserId, userJoinedGames = [] }: GameCardProps) {
  ...
  const timeConflict =
    currentUserId && !hasJoined
      ? checkGameTimeConflict(game, userJoinedGames)
      : { hasConflict: false };
    ...
    <Card className="h-full">
      <CardContent>
        <div className="mb-3">
          ...
          <p className="text-sm text-gray-600 mb-2">Organized by {game.organizer.name}</p>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          {game.status === 'OPEN' && !isFull && <Badge variant="success">Open</Badge>}
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4">
            <div className="flex items-start">
              ...
              <div>
                ...
                <p className="text-yellow-700 text-xs mt-1">{timeConflict.message}</p>
              </div>
            </div>
          </div>
        )}
      ...
      <CardFooter>
        <div className="flex w-full space-x-2">
          ...
          {currentUserId && hasJoined ? (
            ...
            <LeaveGameButton gameId={game.id} gameTitle={game.title} className="flex-1" />
          ) : (
